from src.processing.s3_file_reader import (
    s3_file_reader_local,
    s3_file_reader_remote,
...
class TestS3FileReaderRemote:
    ...
    def test_dummy_ingestion_bucket_contains_test_json_files(
        self, s3_client, dummy_ingestion_bucket
    ):
    ...
    def test_s3_file_reader_remote_extracts_data_from_ingestion_s3(
        self, dummy_ingestion_bucket, s3_client
    ):
        input = test_transaction_data
        result = s3_file_reader_remote(
            "dummy_ingestion_bucket", "test_data.json", s3_client
        )
        # (result)
        assert isinstance(result, pd.DataFrame)
        assert result["transaction_id"][0] == 8
